...
Is when your expected resources are in a different _state_ than your expected _state_

Terraform has three ways to resolve drifts:

- replacing resources: `--replace` can change a resource that has become damaged or degraded (and TF cannot detect it)
- `import` command
- `-refresh-only` command

## Replace resources

Terraform Taint is used can mark a resource for replacement for the next time you run apply
...
> command is now this other `terraform apply -refresh-only (-auto-approve)` (basically the command `terraform refresh` ran behind the scenes)
